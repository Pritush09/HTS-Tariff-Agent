...
def compute_cif(product_cost: float, freight: float, insurance: float) -> float:
    """Compute CIF (Cost + Insurance + Freight) value."""
    return product_cost + freight + insurance

def parse_duty_advanced(duty_str, cif_value, unit_weight=None, quantity=None):
    """
    Parses a duty string and calculates the corresponding duty.
    Returns a fraction of CIF or total duty depending on the type.
    """
...
def calculate_duties_for_dataframe(df, product_cost, freight, insurance, unit_weight, quantity):
    """
    Applies the duty parser to a DataFrame and computes estimated duties.
    """
    ...
    # Return only entries with some duty applicable
    return duty_df[
